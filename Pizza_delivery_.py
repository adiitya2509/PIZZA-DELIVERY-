...
global conn
conn=sqltor.connect(host='localhost',user='root',password='8294')
global c
c=conn.cursor()

'''The following comments needs to be executed before starting the first time implementation of the project
   because they are needed to create the tables once'''
c.execute('CREATE DATABASE IF NOT EXISTS PIZZA')
c.execute('USE PIZZA')
      ...
      def ordernow():
            ...
            try:
                  ...
                  if(getname=="" or getaddress==""or getmob=="" or getmail==""):
                        ...
                        messagebox.showinfo("Order Placed",string)
                        orders.append(x)
                        i=(len(getname)-1)
                        make_list=[x,getname[i],getaddress[i],gettype,getmob,getmail,d]
                        c.execute("INSERT INTO Pizza VALUES({},'{}','{}','{}','{}','{}','{}')",make_list[0],make_list[1],make_list[2],make_list[3],make_list[4],make_list[5])
                        conn.commit()
                        order.destroy()
            except NameError or ValueError or UnboundLocalError:
                  messagebox.showerror("Error","Please enter valid ceredentials")
                  order.destroy()
      ...
      def cancel_now():
            if(enter_order.get()=="" or enter_name.get()==""):
                  ...
                  if(flag==1):
                        ...
                        conn.commit()
                        messagebox.showinfo("Order Cancelled Succesfully","Your Order has been cancelled")
                        ...
                        messagebox.showinfo("Invalid Order id","Order ID you entered is not correct")
                        cancel.destroy()
                        exit
      ...
      def track_now():
            ...
            if(flag==1):
                  ...
                  if(now.day>ttime.day):
                        messagebox.showinfo("Delivered","Your Order is already Delivered")
                        track.destroy()
                  else:
                        if ttime.day==now.day and ttime.hour==now.hour and now.minute<=ttime.minute+20:
                              messagebox.showinfo("Track Order","Your order is Preparing\nIt will be delivered within 20 minutes of order")
                              track.destroy()
                        else:
                              messagebox.showinfo("Track Order","Your order is Ready\nIt will be delivered soon")
                              track.destroy()
            else:
